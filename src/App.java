...
public class App {
    ...
    public void menu() {
        ...
        do {
            ...
            System.out.println("5. Decidir al ganador del premio por ser el mayor productor con stream");
            op = scanner.nextInt();
            switch (op) {
                case 1:
                    Alta();
    ...
    // Con stream
    public void GanadorPremioStream() {
        // Verificar si la lista de empleados está vacía
        if (empleados.isEmpty()) {
            System.out.println("No hay empleados registrados.");
        }
        // Encontrar el empleado con la mayor diferencia de producción
        Empleado mayorProductor = empleados.stream()
                .max(Comparator.comparingInt(empleado -> {
                    // Obtener el valor máximo de producción del empleado actual
                    int maxProduccion = Arrays.stream(empleado.getProduccion()).max().orElse(Integer.MIN_VALUE);
                    // Obtener el valor mínimo de producción del empleado actual
                    int minProduccion = Arrays.stream(empleado.getProduccion()).min().orElse(Integer.MAX_VALUE);
                    // Calcular la diferencia multiplicada por 10
                    return (maxProduccion - minProduccion) * 10;
                }))
                .orElse(null); // Si no se encuentra ningún empleado, retornar null

        // Verificar si se encontró un mayor productor
        if (mayorProductor != null) {
            // Calcular la diferencia de producción nuevamente para imprimir
            int maxProduccion = Arrays.stream(mayorProductor.getProduccion()).max().orElse(Integer.MIN_VALUE);
            int minProduccion = Arrays.stream(mayorProductor.getProduccion()).min().orElse(Integer.MAX_VALUE);
            int diferenciaMaxima = (maxProduccion - minProduccion) * 10;

            // Imprimir el nombre del mayor productor y su diferencia de producción
            System.out.println("El mayor productor es: " + mayorProductor.getNombre());
            System.out.println("Diferencia de producción: " + diferenciaMaxima);
        } else {
            // Si no se encontró ningún empleado, imprimir un mensaje
            System.out.println("No se encontró ningún empleado.");
        }
    }
